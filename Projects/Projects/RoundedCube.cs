    ...
    internal class RoundedCube : ProjectBase
    {
        protected override void RunProject()
        {
            ...
            foreach (var edgeGroup in edgeGroups)
            {
                ...
                var arcA = VectorTransform3D.BarycentricArc(groups[0].A.Normal, groups[1].A.Normal, steps).ToArray();
                var arcB = VectorTransform3D.BarycentricArc(groups[0].B.Normal, groups[1].B.Normal, steps).ToArray();
